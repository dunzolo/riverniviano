...
const fetchMatches = cache(async (tournament: string, filters: Record<string, string | number>) => {
  ...
  const fullUrl = `${BASE_URL}/fe-tournaments/${tournament}/mini-games?${queryString}`;
  ...
  const res = await fetch(fullUrl, {
    next: { tags: ["mini-games"] }, // Cache per 1 ora
  });

  if (!res.ok) {
    console.error("❌ Errore nella fetch Laravel:", res.status, res.statusText);
    throw new Error("Errore nel recupero dei mini giochi");
  }
...
export async function GET(
  ...
  try {
    ...
    console.log("🟢 API Route /api/mini-games called");
